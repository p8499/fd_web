...
public interface DatabaseMapper {
  ...
  @Select(
      "<script><choose><when test='mask!=null'><if test='mask.dbid or mask.dbname or mask.dbstatus or mask.dbmanager or mask.dbspeaker or mask.dborig or mask.dbcrtime or mask.dbmanageralias or mask.dbmanagerfrom or mask.dbmanagername or mask.dbspeakeralias or mask.dbspeakerfrom or mask.dbspeakername or mask.dbdpstatus or mask.dbdpc or mask.dbdpc0 or mask.dbdpc1'><trim prefix='SELECT' suffixOverrides=','><if test='mask.dbid'>DBID, </if><if test='mask.dbname'>DBNAME, </if><if test='mask.dbstatus'>DBSTATUS, </if><if test='mask.dbmanager'>DBMANAGER, </if><if test='mask.dbspeaker'>DBSPEAKER, </if><if test='mask.dborig'>DBORIG, </if><if test='mask.dbcrtime'>DBCRTIME, </if><if test='mask.dbmanageralias'>DBMANAGERALIAS, </if><if test='mask.dbmanagerfrom'>DBMANAGERFROM, </if><if test='mask.dbmanagername'>DBMANAGERNAME, </if><if test='mask.dbspeakeralias'>DBSPEAKERALIAS, </if><if test='mask.dbspeakerfrom'>DBSPEAKERFROM, </if><if test='mask.dbspeakername'>DBSPEAKERNAME, </if><if test='mask.dbdpstatus'>DBDPSTATUS, </if><if test='mask.dbdpc'>DBDPC, </if><if test='mask.dbdpc0'>DBDPC0, </if><if test='mask.dbdpc1'>DBDPC1, </if></trim>FROM F4701View WHERE DBID=#{dbid}</if></when><otherwise>SELECTDBID,DBNAME,DBSTATUS,DBMANAGER,DBSPEAKER,DBORIG,DBCRTIME,DBMANAGERALIAS,DBMANAGERFROM,DBMANAGERNAME,DBSPEAKERALIAS,DBSPEAKERFROM,DBSPEAKERNAME,DBDPSTATUS,DBDPC,DBDPC0,DBDPC1 FROM F4701View WHERE DBID=#{dbid}</otherwise></choose></script>")
  ...
  @Select(
      "<script>SELECT A.* FROM (SELECT B.*, ROWNUM B_ROWNUM FROM (<choose><when test='mask!=null'><trim prefix='SELECT' suffixOverrides=','><if test='mask.dbid'>dbid,</if><if test='mask.dbname'>dbname,</if><if test='mask.dbstatus'>dbstatus,</if><if test='mask.dbmanager'>dbmanager,</if><if test='mask.dbspeaker'>dbspeaker,</if><if test='mask.dborig'>dborig,</if><if test='mask.dbcrtime'>dbcrtime,</if><if test='mask.dbmanageralias'>dbmanageralias,</if><if test='mask.dbmanagerfrom'>dbmanagerfrom,</if><if test='mask.dbmanagername'>dbmanagername,</if><if test='mask.dbspeakeralias'>dbspeakeralias,</if><if test='mask.dbspeakerfrom'>dbspeakerfrom,</if><if test='mask.dbspeakername'>dbspeakername,</if><if test='mask.dbdpstatus'>dbdpstatus,</if><if test='mask.dbdpc'>dbdpc,</if><if test='mask.dbdpc0'>dbdpc0,</if><if test='mask.dbdpc1'>dbdpc1,</if></trim></when><otherwise>SELECT dbid, dbname, dbstatus, dbmanager, dbspeaker, dborig, dbcrtime, dbmanageralias, dbmanagerfrom, dbmanagername, dbspeakeralias, dbspeakerfrom, dbspeakername, dbdpstatus, dbdpc, dbdpc0, dbdpc1</otherwise></choose> FROM F4701View<if test='filter!=null'> WHERE ${filter.toStringOracle()}</if><if test='order!=null'> ORDER BY ${order.toString()}</if>) B WHERE ROWNUM &lt;=#{start}+#{count}) A WHERE B_ROWNUM &gt;=#{start}+1</script>")
  List<Database> query(
